...
gxf_result_t SGMDisparity::start() {
  ...
  impl_->vpi_backends = GXF_UNWRAP_OR_RETURN(vpi::StringToBackend(backend_));
  ...
  GXF_RETURN_IF_ERROR(CheckStatus(vpiInitConvertImageFormatParams(&impl_->disparity_scale_params)));
  ...
  GXF_RETURN_IF_ERROR(CheckStatus(vpiStreamCreate(impl_->vpi_flags, &impl_->stream)));
...
gxf_result_t SGMDisparity::tick() {
  ...
  CameraMessageParts left_image =
      GXF_UNWRAP_OR_RETURN(left_image_receiver_->receive().map(GetCameraMessage));
  CameraMessageParts right_image =
      GXF_UNWRAP_OR_RETURN(right_image_receiver_->receive().map(GetCameraMessage));
  ...
  if (cache_valid) {
    ...
    GXF_RETURN_IF_ERROR(impl_->left_input.update(*left_image.frame));
    GXF_RETURN_IF_ERROR(impl_->right_input.update(*right_image.frame));
    GXF_RETURN_IF_ERROR(impl_->disparity.update(*disparity.frame));
  } else {
    ...
    GXF_RETURN_IF_ERROR(impl_->disparity.createFromVideoBuffer(*disparity.frame, impl_->vpi_flags));
    ...
    GXF_RETURN_IF_ERROR(CheckStatus(vpiImageCreate(
    ...
    // Recreate disparity and confidence VPI image
    ...
    GXF_RETURN_IF_ERROR(CheckStatus(vpiCreateStereoDisparityEstimator(
  ...
  GXF_RETURN_IF_ERROR(CheckStatus(vpiSubmitConvertImageFormat(
  ...
  GXF_RETURN_IF_ERROR(CheckStatus(vpiStreamSync(impl_->stream)));

  // VPI_BACKEND_XAVIER backend only accepts 1920 x 1080 images.
  if (impl_->vpi_backends == vpi::VPI_BACKEND_XAVIER) {
    GXF_RETURN_IF_ERROR(CheckStatus(vpiSubmitRescale(
    ...
    GXF_RETURN_IF_ERROR(CheckStatus(vpiSubmitStereoDisparityEstimator(
  ...
  gxf::Handle<float> max_disparity =
      GXF_UNWRAP_OR_RETURN(disparity.entity.add<float>("max_disparity"));
