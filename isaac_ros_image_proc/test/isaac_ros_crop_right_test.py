...
class IsaacROSCropTest(IsaacROSBaseTest):
    """Validate image cropping in right alligned location."""
    ...
    def test_resize_typical(self, test_folder) -> None:
        ...
        try:
            ...
            yellow_image_cv = np.full(
                (desired_width, desired_height, 3), (0, 255, 255), np.uint8)

            cropped_image_cv = CvBridge().imgmsg_to_cv2(
                cropped_image, desired_encoding='bgr8')

            self.assertImagesEqual(
                cropped_image_cv, yellow_image_cv, threshold_fraction=0)
