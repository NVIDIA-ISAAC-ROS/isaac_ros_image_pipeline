...
"""NV12 test for the Isaac ROS Image Format Converter node."""
...
def generate_test_description():
    ...
    format_container = launch_ros.actions.ComposableNodeContainer(
        ...
        arguments=['--ros-args', '--log-level', 'info',
                   '--log-level', 'isaac_ros_test.image_format_node_rgb8:=debug'],
        output='screen'
    )
...
class IsaacROSFormatNV12Test(IsaacROSBaseTest):
    """Vaidate format conversion to the nv12 format."""

    def test_rgb_to_rgb_conversion(self) -> None:
        """Expect the node to convert rgb8 input images into the rgb8 format."""
