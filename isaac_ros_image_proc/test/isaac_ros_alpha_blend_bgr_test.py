...
"""bgr8 mask encoding test for the Isaac ROS Alpha Blend node."""
...
def generate_test_description():
    ...
    composable_nodes = [
        ComposableNode(
            ...
            namespace=IsaacROSAlphaBlendBgrTest.generate_namespace(),
            parameters=[{
                'alpha': ALPHA
            }])]
    ...
    return IsaacROSAlphaBlendBgrTest.generate_test_description([blend_container])


class IsaacROSAlphaBlendBgrTest(IsaacROSBaseTest):
    filepath = pathlib.Path(os.path.dirname(__file__))

    def test_alpha_blend_bgr(self):
        """
        Test alpha blending feature for bgr8 mask.

        Test that the AlphaBlendNode is correctly blending the image and mask.
        Given:
        - Alpha is 0.7
        - Mask is a bgr8 image with blue values 200, green values 150, red values 100
        - Image is a bgr8 image with blue values 0, green values 100, and red values 200
        The output should be a bgr8 image with:
        - Blue values 0.7 * 0 + 0.3 * 200 = 0 + 60 = 60.
        - Green values 0.7 * 100 + 0.3 * 150 = 70 + 45 = 115.
        - Red values 0.7 * 200 + 0.3 * 100 = 140 + 30 = 170.
        """
        TIMEOUT = 300
        received_messages = {}
        ...
        try:
            ...
            cv_mask = np.zeros((DIMENSION_HEIGHT, DIMENSION_WIDTH, 3), np.uint8)
            cv_mask[:, :, 0] = MASK_BLUE
            cv_mask[:, :, 1] = MASK_GREEN
            cv_mask[:, :, 2] = MASK_RED
