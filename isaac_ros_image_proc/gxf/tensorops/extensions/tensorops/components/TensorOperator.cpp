...
gxf_result_t TensorOperator::doForwardTargetCamera(gxf::Expected<nvidia::gxf::Entity> input_message,
  ...
  return detail::RerouteMessage<gxf::CameraModel>(
...
gxf_result_t TensorOperator::tick() {
  ...
  error =
    detail::RerouteMessage<int64_t>(output_message.value(), input_message.value(),
  ...
  // forwarded (e.g. Undistort module), the function doForwardTargetCamera can be overwritten
  // accordingly
