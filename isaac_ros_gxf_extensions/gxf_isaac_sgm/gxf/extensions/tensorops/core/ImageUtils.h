...
bool Resize(Tensor<HWC, C1, U8>& dst, const Tensor<HWC, C1, U8>& src,
    bool keep_aspect_ratio = false, InterpolationType type = INTERP_LINEAR,
...
bool CropAndResize(Tensor<HWC, C1, U8>& dst, const Tensor<HWC, C1, U8>& src,
    const BBox& dstROI, const BBox& srcROI, InterpolationType type = INTERP_LINEAR,
    cudaStream_t stream = 0);
 ...
 */
bool CropAndResize(Tensor<HWC, C1, U8>& dst, const Tensor<HWC, C1, U8>& src,
    const BBox& srcROI, InterpolationType type = INTERP_LINEAR, cudaStream_t stream = 0);
 ...
 */
bool CropAndResize(Tensor<HWC, C3, F32>& dst, const Tensor<HWC, C3, F32>& src,
    const BBox& srcROI, InterpolationType type = INTERP_LINEAR,
    cudaStream_t stream = 0);
...
bool Crop(Tensor<HWC, C1, U8>& dst, const Tensor<HWC, C1, U8>& src,
    const BBox& srcROI, cudaStream_t stream = 0);
...
bool WarpPerspective(Tensor<HWC, C1, U8>& dst, const Tensor<HWC, C1, U8>& src,
    const double coeffs[3][3], InterpolationType type = INTERP_LINEAR,
    cudaStream_t stream = 0);
...
bool ConvertColorFormat(Tensor<HWC, C3, U8>& dst, const Tensor<HWC, C3, U8>& src,
    ColorConversionType type, cudaStream_t stream = 0);
 ...
 * @param stream specified cuda stream.
 */
bool ConvertBitDepth(Tensor<HWC, C1, U8>& dst, Tensor<HWC, C1, F32>& src, const float scale,
    cudaStream_t stream = 0);
 ...
 * @param stream specified cuda stream.
 */
bool ConvertBitDepth(Tensor<NHWC, C1, U8>& dst, Tensor<NHWC, C1, F32>& src, const float scale,
    cudaStream_t stream = 0);
 ...
 */
bool Normalize(Tensor<HWC, C3, F32>& dst, const Tensor<HWC, C3, U8>& src, const float scale[3],
 ...
 * @stream specified cuda stream.
 */
bool Normalize(Tensor<NHWC, C3, F32>& dst, const Tensor<NHWC, C3, U8>& src, const float scale[3],
    const float offset[3], cudaStream_t stream = 0);
 ...
 * @stream specified cuda stream.
 */
bool Normalize(Tensor<HWC, C3, F32>& dst, const Tensor<HWC, C3, U16>& src, const float scale[3],
    const float offset[3], cudaStream_t stream = 0);
 ...
 * @stream specified cuda stream.
 */
bool Normalize(Tensor<HWC, C3, F32>& dst, const Tensor<HWC, C3, F32>& src, const float scale[3],
    const float offset[3], cudaStream_t stream = 0);
 ...
 * @stream specified cuda stream.
 */
bool Normalize(Tensor<HWC, C1, F32>& dst, const Tensor<HWC, C1, U8>& src, const float scale,
    const float offset, cudaStream_t stream = 0);
 ...
 * @stream specified cuda stream.
 */
bool Normalize(Tensor<NHWC, C1, F32>& dst, const Tensor<NHWC, C1, U8>& src, const float scale,
    const float offset, cudaStream_t stream = 0);
 ...
 * @stream specified cuda stream.
 */
bool Normalize(Tensor<HWC, C1, F32>& dst, const Tensor<HWC, C1, U16>& src, const float scale,
    const float offset, cudaStream_t stream = 0);
 ...
 * @stream specified cuda stream.
 */
bool Normalize(Tensor<NHWC, C1, F32>& dst, const Tensor<NHWC, C1, U16>& src, const float scale,
    const float offset, cudaStream_t stream = 0);
 ...
 * @stream specified cuda stream.
 */
bool Normalize(Tensor<HWC, C1, F32>& dst, const Tensor<HWC, C1, F32>& src, const float scale,
    const float offset, cudaStream_t stream = 0);
 ...
 * @stream specified cuda stream.
 */
bool Normalize(Tensor<NHWC, C1, F32>& dst, const Tensor<NHWC, C1, F32>& src, const float scale,
    const float offset, cudaStream_t stream = 0);
 ...
 * @stream specified cuda stream.
 */
bool Normalize(Tensor<CHW, C1, F32>& dst, const Tensor<CHW, C1, U8>& src, const float scale,
    const float offset, cudaStream_t stream = 0);
 ...
 * @stream specified cuda stream.
 */
bool Normalize(Tensor<NCHW, C1, F32>& dst, const Tensor<NCHW, C1, U8>& src, const float scale,
    const float offset, cudaStream_t stream = 0);
 ...
 * @stream specified cuda stream.
 */
bool Normalize(Tensor<CHW, C1, F32>& dst, const Tensor<CHW, C1, U16>& src, const float scale,
    const float offset, cudaStream_t stream = 0);
 ...
 * @stream specified cuda stream.
 */
bool Normalize(Tensor<NCHW, C1, F32>& dst, const Tensor<NCHW, C1, U16>& src, const float scale,
    const float offset, cudaStream_t stream = 0);
 ...
 * @stream specified cuda stream.
 */
bool Normalize(Tensor<CHW, C1, F32>& dst, const Tensor<CHW, C1, F32>& src, const float scale,
    const float offset, cudaStream_t stream = 0);
 ...
 * @stream specified cuda stream.
 */
bool Normalize(Tensor<NCHW, C1, F32>& dst, const Tensor<NCHW, C1, F32>& src, const float scale,
    const float offset, cudaStream_t stream = 0);
 ...
 * @param stream specified cuda stream.
 */
bool InterleavedToPlanar(Tensor<CHW, C3, U8>& dst, const Tensor<HWC, C3, U8>& src,
    cudaStream_t stream = 0);
...
class ImageToNormalizedPlanarTensorOperator {
 public:
    ...
    void operator()(Tensor<TL_OUT, CC, F32>& dst, const Tensor<TL_IN, CC, CT>& src,
    ...
    void operator()(Tensor<TL_OUT, CC, F32>& dst, const Tensor<TL_IN, CC, CT>& src,
        float scale, float offset, bool keep_aspect_ratio = false, cudaStream_t stream = 0);
