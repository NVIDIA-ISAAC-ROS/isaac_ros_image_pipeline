...
enum ImageType {
    ...
    PLANAR_BGRA_U8,  /**< 8-bit planar BGRA. */
    PLANAR_BGRA_U16, /**< 16-bit planar BGRA. */
    PLANAR_BGRA_F16, /**< half planar BGRA. */
    PLANAR_BGRA_F32, /**< float planar BGRA. */
...
struct ImagePreProcessingParams {
    ImageType imgType;
    //  Image Mean value offset for R,G,B channels. Default is 0.0f
    float pixelMean[3];
    //  Scale or normalization values for R,G,B channels. Default is 1.0/255.0f
    float normalization[3];
    //  Standard deviation values for  R,G,B channels. Default is 1.0f
    float stdDev[3];
};
...
struct IsPlanarImage
    : std::integral_constant<bool, T == PLANAR_RGB_U8 || T == PLANAR_RGB_U16 ||
                                   T == PLANAR_RGB_F16 || T == PLANAR_RGB_F32 ||
                                   T == PLANAR_BGR_U8 || T == PLANAR_BGR_U16 ||
                                   T == PLANAR_BGR_F16 || T == PLANAR_BGR_F32 ||
                                   T == PLANAR_RGBA_U8 || T == PLANAR_RGBA_U16 ||
                                   T == PLANAR_RGBA_F16 || T == PLANAR_RGBA_F32 ||
                                   T == PLANAR_BGRA_U8 || T == PLANAR_BGRA_U16 ||
                                   T == PLANAR_BGRA_F16 || T == PLANAR_BGRA_F32> {
};

template<ImageType T>
struct IsInterleavedImage : std::integral_constant<bool,
    !IsCompositeImage<T>::value && !IsPlanarImage<T>::value> {
};
...
template<>
struct ImageTraits<BGRA_U8, 3> {
    static constexpr TensorLayout TL = TensorLayout::HWC;
    static constexpr ChannelCount CC = ChannelCount::C4;
    static constexpr ChannelType CT  = ChannelType::U8;
};

template<>
struct ImageTraits<BGRA_U8, 4> {
    static constexpr TensorLayout TL = TensorLayout::NHWC;
    static constexpr ChannelCount CC = ChannelCount::C4;
    static constexpr ChannelType CT  = ChannelType::U8;
};
...
inline size_t GetImageElementSize(const ImageType type) {
    ...
    switch (type) {
        ...
        case ImageType::PLANAR_BGRA_U16:
        ...
        case ImageType::PLANAR_BGRA_F16:
            imageElementSize = 2;
            break;
        ...
        case ImageType::PLANAR_BGRA_F32:
            imageElementSize = 4;
            break;
        default:
            imageElementSize = 0;
...
template<>
class Image<ImageType::BGRA_U8> : public Tensor<HWC, C4, U8> {
    using Tensor<HWC, C4, U8>::Tensor;
};

template<>
class Image<ImageType::BGRA_U16> : public Tensor<HWC, C4, U16> {
    using Tensor<HWC, C4, U16>::Tensor;
};

template<>
class Image<ImageType::BGRA_F16> : public Tensor<HWC, C4, F16> {
    using Tensor<HWC, C4, F16>::Tensor;
};

template<>
class Image<ImageType::BGRA_F32> : public Tensor<HWC, C4, F32> {
    using Tensor<HWC, C4, F32>::Tensor;
};
...
template<>
class Image<ImageType::PLANAR_BGRA_U8> : public Tensor<CHW, C4, U8> {
    using Tensor<CHW, C4, U8>::Tensor;
};

template<>
class Image<ImageType::PLANAR_BGRA_U16> : public Tensor<CHW, C4, U16> {
    using Tensor<CHW, C4, U16>::Tensor;
};

template<>
class Image<ImageType::PLANAR_BGRA_F16> : public Tensor<CHW, C4, F16> {
    using Tensor<CHW, C4, F16>::Tensor;
};

template<>
class Image<ImageType::PLANAR_BGRA_F32> : public Tensor<CHW, C4, F32> {
    using Tensor<CHW, C4, F32>::Tensor;
};
...
class Image<ImageType::NV12> {
 public:
    ...
    Image(std::size_t width, std::size_t height, std::uint8_t * dataPtrLuma,
        std::uint8_t * dataPtrChroma, bool isCPU = true)
        : m_data(std::make_tuple(Y(width, height, dataPtrLuma, isCPU),
            UV(width / 2, height / 2, dataPtrChroma, isCPU))) {
        assert(width % 2 == 0 && height % 2 == 0);
    }

    Image(std::size_t width, std::size_t height, std::size_t rowPitchLuma,
        std::size_t rowPitchChroma, std::uint8_t* dataPtrLuma,
        std::uint8_t* dataPtrChroma, bool isCPU = true)
