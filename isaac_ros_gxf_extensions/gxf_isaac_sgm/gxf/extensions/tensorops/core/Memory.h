...
void TensorBaseCopy2D(TensorBase & dst, const TensorBase& src, int dstPitch,
    int srcPitch, int widthInBytes, int height, cudaStream_t stream = 0);
...
template<TensorLayout TL, ChannelCount CC, ChannelType CT,
    typename std::enable_if<TL != HWC && TL != CHW &&
    TL != NHWC && TL != NCHW>::type * = nullptr>
...
void Copy(Tensor<TL, CC, CT>& dst, const Tensor<TL, CC, CT>& src,
 ...
 */
template<TensorLayout TL, ChannelCount CC, ChannelType CT,
    typename std::enable_if<TL == CHW>::type * = nullptr>
void Copy(Tensor<TL, CC, CT>& dst, const Tensor<TL, CC, CT>& src, cudaStream_t stream = 0) {
    TensorBaseCopy2D(dst, src, dst.getStride(TensorDimension::HEIGHT) * GetChannelSize(CT),
        src.getStride(TensorDimension::HEIGHT) * GetChannelSize(CT),
        dst.getWidth() * GetChannelSize(CT),
 ...
 */
template<TensorLayout TL, ChannelCount CC, ChannelType CT,
    typename std::enable_if<TL == NCHW>::type * = nullptr>
void Copy(Tensor<TL, CC, CT>& dst, const Tensor<TL, CC, CT>& src, cudaStream_t stream = 0) {
    TensorBaseCopy2D(dst, src, dst.getStride(TensorDimension::HEIGHT) * GetChannelSize(CT),
        src.getStride(TensorDimension::HEIGHT) * GetChannelSize(CT),
        dst.getWidth() * GetChannelSize(CT),
