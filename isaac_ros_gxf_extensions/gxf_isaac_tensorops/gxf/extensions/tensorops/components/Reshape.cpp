...
gxf_result_t Reshape::doExecute(gxf::Entity& output, gxf::Entity& input,
  ...
  auto result = output_tensor.value()->reshapeCustom(
    output_shape, input_tensor.value()->element_type(),
        gxf::PrimitiveTypeSize(input_tensor.value()->element_type()),
  ...
  if (input_tensor.value()->storage_type() == gxf::MemoryStorageType::kDevice) {
    cudaError_t error = cudaMemcpyAsync(output_tensor.value()->pointer(),
        input_tensor.value()->pointer(),
        input_tensor.value()->size(),
    ...
    memcpy(output_tensor.value()->pointer(), input_tensor.value()->pointer(),
        input_tensor.value()->size());
  }
  return GXF_SUCCESS;
}
