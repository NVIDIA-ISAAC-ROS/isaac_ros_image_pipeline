...
gxf_result_t NormalizeC1Impl(gxf::Entity& output, gxf::Entity& input, const ImageInfo& output_info,
  ...
  auto error = output_adapter->AddImageToMessage<T_OUT>(output,
      output_info.width, output_info.height,
      allocator, output_info.is_cpu, output_name);
  ...
  if (cvcore::tensor_ops::Normalize(output_image.value(), input_image.value(),
      scales[0], offsets[0], stream)) {
    return GXF_SUCCESS;
  } else {
    return GXF_FAILURE;
  }
...
gxf_result_t NormalizeC3Impl(gxf::Entity& output, gxf::Entity& input, const ImageInfo& output_info,
  ...
  auto error = output_adapter->AddImageToMessage<T_OUT>(output, output_info.width,
  ...
  if (cvcore::tensor_ops::Normalize(output_image.value(), input_image.value(),
      scales_value, offsets_value, stream)) {
    return GXF_SUCCESS;
  } else {
    return GXF_FAILURE;
  }
...
#define DEFINE_NORMALIZE_C1(INPUT_TYPE, OUTPUT_TYPE)                        \
  if (input_info_.type == INPUT_TYPE && output_info_.type == OUTPUT_TYPE) { \
    return detail::NormalizeC1Impl<INPUT_TYPE, OUTPUT_TYPE>(                \
        output, input, output_info_, input_info_, output_name,              \
...
#define DEFINE_NORMALIZE_C3(INPUT_TYPE, OUTPUT_TYPE)                        \
  if (input_info_.type == INPUT_TYPE && output_info_.type == OUTPUT_TYPE) { \
    return detail::NormalizeC3Impl<INPUT_TYPE, OUTPUT_TYPE>(                \
        output, input, output_info_, input_info_, output_name,              \
        input_name, output_adapter_.get(), input_adapter_.get(),            \
        pool_.get(), scales_.get(), offsets_.get(), stream);                \
  }

gxf_result_t Normalize::doExecute(gxf::Entity& output, gxf::Entity& input,
    cudaStream_t stream, const char* output_name,
    const char* input_name) {

  // Run the image normalization operation
  DEFINE_NORMALIZE_C1(cvcore::tensor_ops::ImageType::Y_U8,
      cvcore::tensor_ops::ImageType::Y_F32);
  DEFINE_NORMALIZE_C1(cvcore::tensor_ops::ImageType::Y_U16,
      cvcore::tensor_ops::ImageType::Y_F32);
  DEFINE_NORMALIZE_C1(cvcore::tensor_ops::ImageType::Y_F32,
      cvcore::tensor_ops::ImageType::Y_F32);
  DEFINE_NORMALIZE_C3(cvcore::tensor_ops::ImageType::RGB_U8,
      cvcore::tensor_ops::ImageType::RGB_F32);
  DEFINE_NORMALIZE_C3(cvcore::tensor_ops::ImageType::RGB_U16,
      cvcore::tensor_ops::ImageType::RGB_F32);
  DEFINE_NORMALIZE_C3(cvcore::tensor_ops::ImageType::RGB_F32,
      cvcore::tensor_ops::ImageType::RGB_F32);
  DEFINE_NORMALIZE_C3(cvcore::tensor_ops::ImageType::BGR_U8,
      cvcore::tensor_ops::ImageType::BGR_F32);
  DEFINE_NORMALIZE_C3(cvcore::tensor_ops::ImageType::BGR_U16,
      cvcore::tensor_ops::ImageType::BGR_F32);
  DEFINE_NORMALIZE_C3(cvcore::tensor_ops::ImageType::BGR_F32,
      cvcore::tensor_ops::ImageType::BGR_F32);
