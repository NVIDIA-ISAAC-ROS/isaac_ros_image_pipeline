...
gxf_result_t CropAndResizeImpl(gxf::Entity& output,
    ...
    gxf::Handle<gxf::Allocator> allocator, const std::vector<cvcore::tensor_ops::BBox>& src_rois,
  ...
  for (size_t i = 0; i < num_output; i++) {
    std::string output_name_i;
    if (i == 0) {
      // TODO(tazhang) Update once multiple video buffer are supported by CameraMessage
      output_name_i = std::string(output_name);
    } else {
      output_name_i = std::string(output_name) + "_" + std::to_string(i);
    }
    auto error = output_adapter->AddImageToMessage<T>(
    ...
    if (!cvcore::tensor_ops::CropAndResize(output_image.value(),
        input_image.value(), src_rois[i], interp_type, stream)) {
      return GXF_FAILURE;
    }
  }
...
gxf_result_t CropAndResize::registerInterface(gxf::Registrar* registrar) {
  ...
  result &= registrar->parameter(
      input_name_, "input_name", "input name", "input tensor name",
      gxf::Registrar::NoDefaultParameter(), GXF_PARAMETER_FLAGS_OPTIONAL);
  result &= registrar->parameter(
      output_name_, "output_name", "output name", "output tensor name",
      gxf::Registrar::NoDefaultParameter(), GXF_PARAMETER_FLAGS_OPTIONAL);
...
gxf::Expected<ImageInfo> CropAndResize::doInferOutputInfo(gxf::Entity& input) {
  ...
  std::vector<cvcore::tensor_ops::BBox> rois;
  for (size_t i = 0; i < num_bbox; i++) {
    const int index = i * 4;
    rois.push_back({bbox_pointer.value()[index], bbox_pointer.value()[index + 1],
        bbox_pointer.value()[index + 2],
  ...
  return ImageInfo{input_info_.type,
      output_width_.get(), output_height_.get(), input_info_.is_cpu};
}
...
gxf_result_t CropAndResize::doUpdateCameraMessage(gxf::Handle<gxf::CameraModel>& output,
  ...
  *output = GetScaledCameraModel(crop_result.value(),
      output_info_.width, output_info_.height, false).value();
  return GXF_SUCCESS;
}

#define DEFINE_CROP_AND_RESIZE(INPUT_TYPE)                                    \
  if (input_info_.type == INPUT_TYPE) {                                       \
    return detail::CropAndResizeImpl<INPUT_TYPE>(output, input, output_info_, \
        input_info_, output_name, input_name,                                 \
        output_adapter_.get(), input_adapter_.get(), pool_.get(),             \
        input_rois_, interp.value(), stream);                                 \
  }

gxf_result_t CropAndResize::doExecute(gxf::Entity& output,
  ...
  DEFINE_CROP_AND_RESIZE(cvcore::tensor_ops::ImageType::Y_U8);
  DEFINE_CROP_AND_RESIZE(cvcore::tensor_ops::ImageType::Y_U16);
  DEFINE_CROP_AND_RESIZE(cvcore::tensor_ops::ImageType::Y_F32);
  DEFINE_CROP_AND_RESIZE(cvcore::tensor_ops::ImageType::RGB_U8);
  DEFINE_CROP_AND_RESIZE(cvcore::tensor_ops::ImageType::RGB_U16);
  DEFINE_CROP_AND_RESIZE(cvcore::tensor_ops::ImageType::RGB_F32);
  DEFINE_CROP_AND_RESIZE(cvcore::tensor_ops::ImageType::BGR_U8);
  DEFINE_CROP_AND_RESIZE(cvcore::tensor_ops::ImageType::BGR_U16);
  DEFINE_CROP_AND_RESIZE(cvcore::tensor_ops::ImageType::BGR_F32);
