...
#include "extensions/tensorops/core/OneEuroFilter.h"
...
template<class T>
struct deduceDataType<T, typename std::enable_if<std::is_same<T, float>::value ||
    std::is_same<T, Vector2f>::value ||
    std::is_same<T, Vector3f>::value>::type> {
    typedef float U;
};
template<class T>
struct deduceDataType<T, typename std::enable_if<std::is_same<T, double>::value ||
...
struct OneEuroFilterState {
    ...
    std::error_code resetParams(const OneEuroFilterParams& filterParams) {
        if (filterParams.dataUpdateRate <= 0.0f || filterParams.minCutoffFreq <= 0 ||
            filterParams.derivCutoffFreq <= 0) {
    ...
    std::error_code filter(U& outValue, U value) {
        m_prevfilteredValue = m_currfilteredValue;
        U dxValue           = xFilt->isInitialized()
            ? (value - xFilt->getPreviousValue()) * m_freq : 0.0f;
        U edxValue;
...
struct OneEuroFilter<T>::OneEuroFilterImpl {
    ...
    OneEuroFilterImpl(const OneEuroFilterParams& filterParams) {
        size_t numStates = get_dim<T>::value;
    ...
    template<typename U = T, typename std::enable_if<get_dim<U>::value == 1>::type * = nullptr>
    std::error_code filter(U& outValue, U value) {
        if (m_states.size() != 1) {
