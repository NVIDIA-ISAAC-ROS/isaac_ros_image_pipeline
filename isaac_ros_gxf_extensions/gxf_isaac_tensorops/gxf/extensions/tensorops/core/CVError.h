...
// which is mapped to the cvcore::tensor_ops::ErrorCode.
// As an example, cvcore::tensor_ops::ErrorCode -1 may not ABSOLUTELY mean the same as
// tensor_ops::FaultCode -1, but does mean the same as tensor_ops:FaultCode 4.
// Thus, tensor_ops::FaultCode 4 needs to be mapped to cvcore::tensor_ops::ErrorCode -1.
...
namespace std {
template<>
struct is_error_condition_enum<cvcore::tensor_ops::ErrorCode> : true_type {
};

template<>
struct is_error_code_enum<cvcore::tensor_ops::ErrorCode> : true_type {
};
...
inline void CheckErrorCode(std::error_code err, const char * file, const int line) {
    const std::string message = "Error returned at " + std::string(file) + ":" +
        std::to_string(line) + ", Error code: " + std::string(err.message());
    if (err != cvcore::tensor_ops::make_error_code(cvcore::tensor_ops::ErrorCode::SUCCESS)) {
        throw std::runtime_error(message);
    }
}
...
#define CHECK_ERROR(val)                                   \
    {                                                      \
        cvcore::tensor_ops::CheckCudaError((val), __FILE__, __LINE__); \
    }

#define CHECK_ERROR_CODE(val)                               \
    {                                                    \
        cvcore::tensor_ops::CheckErrorCode((val), __FILE__, __LINE__); \
    }
