...
class VPITensorStream::VPIStereoDisparityEstimatorImpl {
 public:
    ...
    std::error_code initialize(Image<T_OUT> & outImage, const Image<T_IN> & leftImage,
        const Image<T_IN> & rightImage, VPIBackend backend);
    ...
    * @param leftImage StereoDisparityEstimator input left image
    * @param rightImage StereoDisparityEstimator input right image
    * @param windowSize Represents the median filter size (on PVA+NVENC_VIC backend)
    ...
    * @param backend VPI backend used for StereoDisparityEstimator
    * @return Success if StereoDisparityEstimator is submitted successfully,
    *         otherwise error is returned
    */
    template<ImageType T_OUT, ImageType T_IN>
    std::error_code execute(Image<T_OUT> & outImage, const Image<T_IN> & leftImage,
        const Image<T_IN> & rightImage, size_t windowSize, size_t maxDisparity,
