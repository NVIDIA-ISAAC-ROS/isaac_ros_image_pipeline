...
def generate_test_description():
    point_cloud_xyz_node = ComposableNode(
        ...
        namespace=IsaacROSDepthImageProcPointCloudXyzOSSTest.generate_namespace(),
    ...
    # Reference Node
    reference_point_cloud_xyz_node = ComposableNode(
        name='reference_point_cloud_xyz_node',
    ...
    container = ComposableNodeContainer(
        ...
        composable_node_descriptions=[point_cloud_xyz_node,
                                      reference_point_cloud_xyz_node],
        output='screen'
    )

    return IsaacROSDepthImageProcPointCloudXyzOSSTest.generate_test_description(
        [container])


class IsaacROSDepthImageProcPointCloudXyzOSSTest(IsaacROSBaseTest):
            ...
            for isaac_ros_msg, ref_msg in zip(
                ...
                for isaac_ros_pt, ref_pt in zip(isaac_ros_pts, ref_pts):
                    ...
                    isaac_ros_pt = np.array(
                        [isaac_ros_pt[0], isaac_ros_pt[1], isaac_ros_pt[2]])
                    ref_pt = np.array(
