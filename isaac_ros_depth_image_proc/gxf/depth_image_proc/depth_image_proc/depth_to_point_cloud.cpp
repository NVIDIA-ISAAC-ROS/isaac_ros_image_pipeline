...
gxf_result_t DepthToPointCloud::tick()
{
  ...
  gxf::Entity depth_entity = GXF_UNWRAP_OR_RETURN(depth_receiver_->receive());
  nvidia::isaac::CameraMessageParts depth_message = GXF_UNWRAP_OR_RETURN(nvidia::isaac::GetCameraMessage(depth_entity));
  GXF_RETURN_IF_ERROR(validateDepthMessage(depth_message));

  nvidia::isaac::CameraMessageParts image_message;
  if (colorize_point_cloud_) {
    gxf::Entity image_entity = GXF_UNWRAP_OR_RETURN(image_receiver_.try_get().value()->receive());
    image_message = GXF_UNWRAP_OR_RETURN(nvidia::isaac::GetCameraMessage(image_entity));
    GXF_RETURN_IF_ERROR(validateImageMessage(depth_message, image_message));
  }

  // Create new message
  PointCloudProperties point_cloud_properties = createPointCloudProperties(depth_message, skip_.get());
  nvidia::isaac_ros::messages::PointCloudMessageParts point_cloud_message = GXF_UNWRAP_OR_RETURN(
