...
constexpr char INPUT_DEPTH_TOPIC_NAME[] = "depth_registered/image_rect";
...
constexpr char INPUT_RGB_IMAGE_TOPIC_NAME[] = "rgb/image_rect_color";

constexpr char INPUT_CAMERA_INFO_COMPONENT_KEY[] = "sync/cam_info_receiver";
constexpr char INPUT_CAMERA_INFO_FORMAT[] = "nitros_camera_info";
constexpr char INPUT_CAMERA_INFO_TOPIC_NAME[] = "rgb/camera_info";
...
constexpr char APP_YAML_FILENAME[] = "config/nitros_point_cloud_xyzrgb_node.yaml";
  ...
  output_width_(declare_parameter<uint16_t>("output_width", 1920))
{
  ...
  for (auto & config : config_map_) {
    if (config.second.topic_name == INPUT_DEPTH_TOPIC_NAME ||
      config.second.topic_name == INPUT_RGB_IMAGE_TOPIC_NAME ||
      config.second.topic_name == INPUT_CAMERA_INFO_TOPIC_NAME)
    {
...
void PointCloudXyzrgbNode::postLoadGraphCallback()
{
  RCLCPP_DEBUG(get_logger(), "[PointCloudXyzrgbNode] postLoadGraphCallback().");
